...
parser.add_argument('--target_file_prefix', help='prefix of the target file we are creating into the s3 or fs', action='store', default='')
...
def copy_to_s3(tar_name, org_files_list):
    ...
    # uploading to S3
    s3_client.upload_fileobj(recv_buf, bucket_name, tar_name, ExtraArgs={'Metadata': {'tar_archived': 'true'},'StorageClass': storage_class}, Config=transfer_config)
    ### print metadata
    ...
    while True:
        ...
        try:
            if protocol == 's3':
                ...
                print("invalid protocol, input s3 or fs")
                exit(100)
        except Exception as e:
